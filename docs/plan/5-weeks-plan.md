...
> **참고**: local/test 프로파일은 `ddl-auto: update`이므로 Entity `@Table(indexes = ...)` 어노테이션을 추가하면 앱 시작 시 인덱스가 자동 생성된다. 별도 DDL 실행 불필요.
...
class ProductCacheRepository(
    ...
    fun getProductList(brandId: Long?, sort: String, page: Int, size: Int): CachedPage<ProductInfo>? {
        return try {
            ...
            val json = redisTemplate.opsForValue().get(key) ?: return null
            objectMapper.readValue(json, object : TypeReference<CachedPage<ProductInfo>>() {})
        } catch (e: Exception) {
            null
        }
    }
...
> **Page 직렬화 문제**: Spring의 `PageImpl`은 기본 역직렬화가 안 됨. `CachedPage` (캐시 전용 data class)를 만들어서 JSON 직렬화/역직렬화를 해결. `PageImpl` 상속 방식(RestPageImpl)보다 Spring 의존 없는 순수 data class가 더 깔끔함.

#### 새 파일: `CachedPage.kt`

- **위치**: `apps/commerce-api/src/main/kotlin/com/loopers/support/cache/CachedPage.kt`
- Kotlin data class로 Jackson 직렬화/역직렬화가 자연스럽게 동작
- `toPage()`로 Spring Page 변환, `from()`으로 Page → CachedPage 변환
...
class ProductService(
    ...
    fun getAllProducts(brandId: Long?, pageable: Pageable): Page<ProductInfo> {
        ...
        // 1. 캐시 조회
        productCacheRepository.getProductList(brandId, sort, page, size)?.let { return it.toPage() }
        ...
        // 3. 캐시 저장
        productCacheRepository.setProductList(brandId, sort, page, size, CachedPage.from(result))
...
```
Step 1: ProductSortType enum 생성 + Controller 정렬 파라미터 추가       ✅ 완료
...
Step 2: 복합 인덱스 추가                                                ✅ 완료
        ...
        - (ddl-auto: update이므로 별도 DDL 스크립트 불필요)

Step 3: 정렬 기능 테스트 작성 (단위 + 통합 + E2E)                       ✅ 완료
        → 커밋: feat: 상품 목록 정렬 기능 및 복합 인덱스 구현
        → 커밋: test: 상품 목록 정렬 테스트 추가

Step 4: Redis 캐시 구현 — 상품 상세 + 목록                              ✅ 완료
        - CachedPage.kt 생성 (캐시 전용 DTO)
        - ProductCacheRepository.kt 생성
        - ProductService 캐시 연동 + 무효화
        → 커밋: feat: 상품 API Redis 캐시 적용

Step 5: 캐시 테스트 작성 (단위 + 통합 + E2E)                            ✅ 완료
        → 커밋: test: 상품 캐시 테스트 추가

Step 6: 10만건 테스트 데이터 SQL 스크립트 생성
        - data/seed-products.sql 작성

Step 7: EXPLAIN 분석 (인덱스 전/후 비교)
        - 로컬에서 실행, 결과를 블로그에 기록

Step 8: 단계별 부하 테스트 (Phase 1~4)
        - Phase 1: 인덱스 없이 Baseline
        - Phase 2: 인덱스 + 비정규화 적용
        - Phase 3: 캐시 적용
        - Phase 4: 캐시 miss + 인덱스 유무 비교
```
...
| `support/cache/CachedPage.kt` | **신규** — 캐시 전용 Page DTO (PageImpl 직렬화 문제 해결) |
...
### 구현 시 주의사항
- local/test 프로파일은 `ddl-auto: update`이므로 Entity 어노테이션 기반 인덱스 자동 생성
- Redis 장애 시 서비스 중단 방지 (모든 캐시 접근에 try-catch)
- `keys()` 명령은 프로덕션에서 성능 이슈 가능 — 현재는 학습 목적이므로 사용하되, 실무에서는 SCAN 또는 별도 키 관리 전략 필요
- Page 직렬화를 위한 `CachedPage` 캐시 전용 DTO 사용 (RestPageImpl 상속 방식 대신 순수 data class 선택)
